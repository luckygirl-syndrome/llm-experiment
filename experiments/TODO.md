...
| 5-2 | `02_generate_users.py` — 가상 유저 80명 생성 | ⬜ | |
...
## Phase 2: 시험 1 (Vision 정확도 + 마케팅 트리거)

> **변경 (2026-04-15):** 기존 시험 2(마케팅 트리거)를 시험 1에 흡수.
> 시험 0 프롬프트 v2에서 마케팅 트리거도 동시 추출 → 같은 100장 GT로 평가.
...
| 3-2 | `01_build_test_set.py` — 시험 0 결과에서 옷 묘사 20종 추출 + 조합 생성 | ⬜ | 120개 조합 |
| 3-3 | `prompts/style_similarity.txt` — docs에서 프롬프트 추출 | ⬜ | |
| 3-4 | `02_run_scoring.py` — Gemini 360회 호출 | ⬜ | temperature=0, 3회 반복 |
| 3-5 | `03_analyze.py` — 분포, 일관성, 방향성 분석 | ⬜ | |
...
| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| 4-1 | `configs/scenarios.yaml` — 9개 시나리오 작성 (UM 유형 시나리오 추가 필요) | ⬜ | 4분면 × 2유형 + 태도 시나리오 |
| 4-1b | prompt_builder 실험 조건 설계 (축 설명 개요/상세, 유형 설명, 축+긴장관계) | ⬜ | 긴장관계 평가용 유형 1개 확정 필요 |
| 4-2 | `01_build_scenarios.py` — 점수 계산 + 프롬프트 조립 | ⬜ | |
| 4-3 | `02_run_conversations.py` — Gemini로 대화 실행 | ⬜ | 발화 3종 × 시나리오 9개 × 유형 2개 |
| 4-4 | `03_evaluate.py` — 자동 체크 + 수동 평가 템플릿 + 최종 보정 코드 평가 | ⬜ | |
...
| U-6 | UM 유형 시나리오 추가 (시험 4: 축간 긴장 검증) | TBD | Phase 4 전 | ⬜ |
| U-7 | .env에 GEMINI_API_KEY 설정 | 팜팜이 | 즉시 | ✅ |
| U-8 | 최종 보정 코드 평가 방식 결정 + 유저 태도 시나리오 설계 | TBD | Phase 4 전 | ⬜ |
| U-9 | 긴장관계 평가용 유형 1개 확정 (IT or UM) | TBD | Phase 4 전 | ⬜ |
